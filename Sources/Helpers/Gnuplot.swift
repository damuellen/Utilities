...
public final class Gnuplot: CustomStringConvertible {
  ...
  @discardableResult public func plot(
    ...
    let (s, l) = style.raw
    multiplot += multi ? 1 : 0
    if styles.isEmpty { styles = Array(stride(from: 11, through: 17, by: 1)).shuffled() }
    let command =
  ...
  public init<T: FloatingPoint>(y1s: [[T]], y2s: [[T]] = [], titles: [String] = [], range: DateInterval) {
    ...
    self.defaultPlot = "plot " + y1s.indices.map { i in
      "$data i \(i) u ($0*\(range.duration / Double(y1s[i].count))):\(1) axes x1y1 w l ls \(i+11) title columnheader(1)"
      }.joined(separator: ", \\\n") + (y2s.isEmpty ? "" : ", \\\n") + y2s.indices.map { i in
      "$data i \(i + y1s.endIndex) u ($0*\(range.duration / Double(y2s[i].count))):\(1) axes x1y2 w l ls \(i+y1s.endIndex+11) title columnheader(1)"
      }.joined(separator: ", \\\n")
  }
